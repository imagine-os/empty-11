...
| `2xl` | 2560 | 1440p / QHD desktop | 21.41% and rising (Steam, Aug 2026) — big enough a slice, with different UI density needs than 1080p, to deserve its own tier rather than being lumped into "everything ≥1920". |
| `3xl` | 3840 | 4K desktop workstation *and* TV/kiosk | 4K desktop just passed 5% (StatCounter/Steam) and 4K is now the standard panel for TVs ≥43"; both share the same CSS pixel width, so one width tier serves both — device *class* (below), not width, is what tells them apart. |
