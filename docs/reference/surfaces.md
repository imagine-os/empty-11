...
| CLI | `pnpm --filter @paperos/agents tree [--check] [--plain] [--plan <file>] [<dir>]` — print the roster org tree with model / effort / mode and budgets, diff its structure against plan.json; `--check` exits 1 on drift | PAP-284 | - | live |
| CLI | `pnpm --filter @paperos/agents plan-to-roster [--plan <file>] [--out <dir>] [--merge] [--check] [--force] [--dry-run]` — convert plan.json `agents[]` to `roster.yaml` + `characters/*.yaml`; `--merge` keeps hand edits, `--check` names drift | PAP-284 | - | live |
| CLI | `pnpm --filter @paperos/agents validate` with no argument now validates the live roster (`packages/agents/roster.yaml` + `characters/`) before `fixtures/valid` | PAP-284 | - | live |
| API | `@paperos/agents/roster` — `convertPlan`, `mergeCharacter`, `characterDrift`, `renderTree`, `diffAgainstPlan`, `validateLiveRoster`, `readLiveRosterFiles`, policy constants (`ROSTER_DEFAULTS`, `LEAD_SHARES`, `READ_MOSTLY_SUBS`) | PAP-284 | - | live |
